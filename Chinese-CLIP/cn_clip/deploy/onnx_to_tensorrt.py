...
from tensorrt_utils import TensorRTShape
...
def build_engine(onnx_file_path, input_shapes, logger, workspace_size, fp16, int8):
    # 创建构建器
    ...
    try:
        ...
        print(f"ONNX 文件 {onnx_file_path} 未找到")
        return None
    ...
    # 设置输入形状
    for tensor_shape in input_shapes:
        input_name = tensor_shape.input_name
        shape = tensor_shape.optimal_shape
        profile = builder.create_optimization_profile()
        profile.set_shape(input_name, min=tensor_shape.min_shape, opt=shape, max=tensor_shape.max_shape)
